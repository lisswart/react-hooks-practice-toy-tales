...
Then, run `npm run server` to start up `json-server` on `http://localhost:6001`.
...
[x] _When the `Donate to Goodwill` button is clicked_, make a DELETE request to
...
[x] _When the like button is clicked_, make a PATCH request to `/toys/:id` with
